...
void AMarchingSquares::BeginPlay()
{
	...
	//TArray<FVector2D> test = {FVector2D(0, 100), FVector2D(0, 50), FVector2D(50, 100)};
	//CreateShape(test, FVector2D(0, 0));
	//CreateShape(test, FVector2D(0, 50));

	//FActorSpawnParameters spawnParameters;
	//GetWorld()->SpawnActor<AActor>(m_blackCube, FVector(-1000, -1000, 50), FRotator(0, 0, 0),  spawnParameters);

	//SpawnCaseActors(15, FVector2D(-1500, -1000));
}
...
{
	for (int i = 0; i < (m_CA->m_gWidth - 1); i++)
	{
		for (int j = 0; j < (m_CA->m_gHeight - 1); j++)
		{
			float a = m_CA->m_gGrid[i][j];
			float b = m_CA->m_gGrid[i + 1][j];
			float c = m_CA->m_gGrid[i + 1][j + 1];
			float d = m_CA->m_gGrid[i][j + 1];
...
switch (total)
	...
	case 5:
		...
		UE_LOG(LogTemp, Warning, TEXT("case5"));

		break;

	case 6:

		UE_LOG(LogTemp, Warning, TEXT("case6"));

		break;

	case 7:

		UE_LOG(LogTemp, Warning, TEXT("case7"));

		break;

	case 8:

		UE_LOG(LogTemp, Warning, TEXT("case8"));

		break;

	case 9:

		UE_LOG(LogTemp, Warning, TEXT("case9"));

		break;

	case 10:

		UE_LOG(LogTemp, Warning, TEXT("case10"));

		break;

	case 11:

		UE_LOG(LogTemp, Warning, TEXT("case11"));

		break;

	case 12:

		UE_LOG(LogTemp, Warning, TEXT("case12"));

		break;

	case 13:

		UE_LOG(LogTemp, Warning, TEXT("case13"));

		break;

	case 14:

		UE_LOG(LogTemp, Warning, TEXT("case14"));

		break;

	case 15:

		//SpawnCaseActors(15, FVector2D(offsetX, offsetY));

		//FActorSpawnParameters spawnParameters;

		//TSubclassOf<AActor> actor = m_actorToSpawn;
		//GetWorld()->SpawnActor<AActor>(m_blackCube, FVector(offsetX * 100, offsetY * 100, 50), FRotator(0, 0, 0),  spawnParameters);

		//UStaticMeshComponent* staticMesh = CreateDefaultSubobject<UStaticMeshComponent>(FName("StaticMesh"));
		//staticMesh->SetupAttachment(actor.);
		//staticMesh->SetStaticMesh(m_staticMesh[15]);

			UE_LOG(LogTemp, Warning, TEXT("case15"));
	...
	case 1:
		...
		//CreateShape(localVertices, FVector2D(offsetX, offsetY));
	...
	case 15:
		...
		TArray<FVector2D> tempLocalVertices = {FVector2D(100, 0), FVector2D(100, 100), FVector2D(0, 100), FVector2D(0, 0)};
		localTriangles = {0, 1, 2, 0, 2, 3};
		
		UE_LOG(LogTemp, Warning, TEXT("case 15"));
